...
module TextHelpers
  ...
  def replace_external_links(content)
    return "" if content.blank?
    # 1. controllare tutti i tag a
    # 2. Se il tag a continene un link a un dominio diverso da BASE_URL
    # 3. Aggiungere un attributo target="_blank"
    # 4. Se il link è == BASE_URL ma è un pdf
    # 5. oltre a target, aggiungere un attributo aria-label="È un pdf"
